...
contract Pool is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
 ...
 if (!tokenIsAllowed(tokenAddress, tokensForLending)) {
  ...
  function payDebt(address tokenAddress, uint256 amount) external nonReentrant {
    ...
    uint256 totalTokenAmountToCollectFromUser = amount + interest(tokenAddress, amount);
  ...
  function withdraw(address tokenAddress, uint256 amount) external nonReentrant {
    ...
    uint totalTokenSuppliedInContract = getTotalTokenSupplied(tokenAddress) * 10 ** decimals;
    uint totalTokenBorrowedInContract = getTotalTokenBorrowed(tokenAddress) * 10 ** decimals;
    ...
    for (uint256 j = 0; j < tokensForLending.length; j++) {
      ...
      if(currentTokenForLending.tokenAddress == tokenAddress) {
        uint256 amountOut = amount + (amount * currentTokenForLending.stableRate) / 1e18;
  ...
  function getUserTotalAmountAvailableForBorrowInDollars(
    ...
    for (uint256 i = 0; i < lenders.length; i++) {
      ...
      if (currentLender == user) {
        for (uint256 j = 0; j < tokensForLending.length; j++) {
          Token memory currentTokenForLending = tokensForLending[j];
          uint256 currentTokenLentAmount = tokensLentAmount[currentTokenForLending.tokenAddress][
            user
          ];
          ...
          uint256 availableInDollar = (currentTokenLentAmountInDollar *
            currentTokenForLending.LTV) / 1e18;
